...
| Concept | Key Principle | Engineering Impact | Primary Subsystem | Design Guidance | Related Chapters |
|---------|---------------|--------------------|-------------------|-----------------|------------------|
| Thermal Noise | Johnson–Nyquist thermal noise | Sets the minimum achievable noise floor | ADC, Analog Front-End | Use lower resistance values and limit bandwidth where practical | 8, 11 |
| Quantization Noise | ADC quantization | Limits achievable resolution and dynamic range | ADC, DAC | Increase resolution, oversample where appropriate | 11 |
| RC Filtering | First-order RC low-pass filter | Removes high-frequency noise and prevents aliasing | Analog Front-End | Select component values to achieve the desired cutoff frequency | 8, 11 |
| Clock Jitter | Sampling clock timing uncertainty | Reduces SNR at higher input frequencies | ADC, Clock System | Use stable, low-jitter clock sources | 11 |
| Ground Bounce | Inductive voltage generated by switching currents | Introduces measurement error and digital noise | Power Distribution, ADC | Minimize loop area and use proper decoupling | 8 |
| Aliasing | Undersampling of high-frequency signals | Produces incorrect frequency components | ADC | Use anti-aliasing filters before sampling | 11 |
| Resistor Drift | Temperature-dependent resistance variation | Causes gain and offset drift | Analog Front-End, DAC | Use precision, low-temperature-coefficient resistors | 8 |
| Capacitor Charging | RC charging and settling behavior | Determines acquisition and settling time | ADC Sample-and-Hold | Ensure acquisition time is significantly greater than the RC time constant | 8 |
